...
let rec loop ~copy_input ~syntax ~f ~file_contents ~keep ~text_start ~bol ~pos =
  ...
  | Some groups ->
    ...
    | Some groups ->
      ...
      loop ~copy_input ~syntax ~f ~file_contents ~keep:true ~pos:stop ~bol ~text_start:stop
    | None ->
      let bol = advance_bol file_contents bol ~start:pos ~stop:stop in
      ...
      loop ~copy_input ~syntax ~f ~file_contents ~keep:keep_next ~text_start:pos ~bol ~pos
...
let run ~syntax ~f ~file_name ~file_contents ~copy_input =
  loop ~copy_input ~syntax ~f ~file_contents ~keep:true ~text_start:0
